...
file name: chatting_usr2.c
brief: Simple chat program using message queue, user2.
author: github_174cm
*/
#include <sys/types.h>
#include <mqueue.h>
...
void *send(void *arg)
{
    while(1) {
        ...
        if (mq_send(qd2, send_data, strlen(send_data), PRIORITY) == -1) {
...
{
    while(1) {
        if (mq_receive(qd, recv_data, BUFSIZE, 0) == -1) {
                perror ("mq_send failed");
                exit(1);
        }
        printf("%s receive\n", recv_data);
